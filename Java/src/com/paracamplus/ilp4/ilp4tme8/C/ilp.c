...
/** find field by name=property*/
ILP_Object
ILP_lookup_property(ILP_Object target, char* property)
{
     if (ILP_is_a(target, &ILP_object_Object_class)){
          ILP_Class class = target->_class;
          ILP_Field last = class->_content.asClass.last_field;
          short index = -1;

          while(last){
               if(!strcmp(last->_content.asField.name, property)){
                    index = last->_content.asField.offset;
               }
               last = last->_content.asField.previous_field;
          }

          if(index>=0)
               return target->_content.asInstance.field[index];
          
          return NULL;
     }
     else{
          return ILP_domain_error("target is not an object", target);
     }
}

ILP_Object
ILP_has_property(ILP_Object target, ILP_Object property){
     if(ILP_isString(property)){
          
          return ILP_make_boolean(ILP_lookup_property(target, property->_content.asString.asCharacter)!=NULL);
     }
     else{
          return ILP_domain_error("Property is not a string", target);
     }
}
